...
#Function: SubsetLevels 
 #   SubsetLevels <- function(metatable, condition_indices, keep_or_exclude, condition_numbers){
 #   for( i in 1:length(condition_indices)){
 #     list_final <- c() 
 #     Levels_Cdtn <- levels(droplevels(as.factor(metatable[,condition_indices[i]])))
 #     subset_meta <- data.frame(Index=1:length(Levels_Cdtn),Levels_Cdtn)
 #     colnames(subset_meta)[2] <- paste("Levels_",colnames(metatable[condition_indices[i]]))
 #     IRdisplay::display(subset_meta)
 #     
 #     Read_cdtn <- toupper(keep_or_exclude[i])
 #     if( Read_cdtn=="K"){
 #       temp <- condition_numbers[[i]]
 #       ty <- class(metatable[,condition_indices[i]])
 #       list_keep <-Levels_Cdtn[temp, drop=F]
 #       list_exc <-Levels_Cdtn[-temp, drop=F]
 #       cat("The condition(s) you want to exclude in ",colnames(metatable)[condition_indices[i]]," :",list_exc,"\n")
 #       cat("The condition(s) you want to keep in ",colnames(metatable)[condition_indices[i]]," :",list_keep,"\n")
 #       
 #     }else if(Read_cdtn=="E"){
 #       temp <- condition_numbers[[i]]
 #       ty <- class(metatable[,condition_indices[i]])
 ...
 #      #Gets all the elements in list_keep into list_final
 #     for(j in 1:length(list_keep)){
 #       sub_list <- metatable[(metatable[,condition_indices[i]] == list_keep[j]),]
 #       list_final <- rbind(list_final,sub_list)
 #     }
 #     metatable <- list_final
 #   }
 #   return(metatable)
 # }
